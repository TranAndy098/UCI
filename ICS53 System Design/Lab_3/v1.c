...
page page_table[pg_table_size];
int main_memory[mm_size];
int main_memory_table[mm_pages];
int main_memory_usage[mm_pages];
...
{   
    ...
    while (!quit) 
    {
        ...
        else if (strcmp(token, "showmain")==0) 
        {
            ...
            if (main_memory_table[page_number] == -1) {
                printf("%d does not exist in main memory\n", page_number);
            }
            else {
        ...
        else if (strcmp(token, "read")==0) 
        {
            ...
            if (addr > 128) {
                ...
                printf("read %s is page %d with offset %d\n",token, page_number, offset);
        ...
        else if (strcmp(token, "write")==0) 
        {
            ...
            if (addr > 128) {
                ...
                token = strtok(NULL, "     ");
                value = atoi(token);
                printf("write %d to page %d with offset %d\n", value, page_number, offset);
...
void not_in_main(int page_number) {
    int main_place = replace_check(page_number);
        if (main_place == -1) {
            ...
            if (old_page == -1) {
                printf("Error has occured could not find page to replace\n");
                return;
            }
...
void update_usage(int page_number) {
    ...
    if (lru) {
        ...
        main_memory_usage[3] = page_number;
    }
}