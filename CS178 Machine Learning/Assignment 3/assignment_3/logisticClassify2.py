...
## METHODS ################################################################
    ...
    def plotBoundary(self,X,Y):
        ...
        x2b = np.array([b2x, b2y]);      # TODO find x2 values as a function of x1's values
    ...
    def train(self, X, Y, initStep=1.0, stopTol=1e-4, stopEpochs=5000, plot=None):
        ...
        if N==2: plt.figure(2); self.plotBoundary(X,Y); plt.draw(); # & predictor if 2D
        plt.pause(.01); # let OS draw the plot
        print("The loss for train is:", Jnll[-1])
    
    def trainL1(self, X, Y, alpha = 2, initStep=1.0, stopTol=1e-4, stopEpochs=5000, plot=None):
        """ Train the logistic regression using stochastic gradient descent with L1 regularization"""
        M,N = X.shape;                     # initialize the model if necessary:
        self.classes = np.unique(Y);       # Y may have two classes, any values
        XX = np.hstack((np.ones((M,1)),X)) # XX is X, but with an extra column of ones
        YY = ml.toIndex(Y,self.classes);   # YY is Y, but with canonical values 0 or 1
        ...
        while not done:
            ...
            for i in np.random.permutation(M):
                ...
                #derive_nll = sigma_r - YY[i]
                derive_nll = -YY[i] * (1 - sigma_r) + (1 - YY[i]) * sigma_r
                regularization_vector = alpha * np.array([1 if i > 0 else -1 for i in self.theta])
                gradi = np.array([derive_nll, derive_nll * X[i,0], derive_nll * X[i,1]])
                gradi += regularization_vector              
                self.theta -= stepsize * gradi;  # take a gradient step
            ...
            for i in range(X.shape[0]):
                ...
                if YY[i] > 0:
                    ...
                    Jsur += -np.log(1 - sigma_r)
            Jsur = Jsur / M
            Jsur += alpha * np.sum(np.abs(self.theta))
        ...
        if N==2: plt.figure(2); self.plotBoundary(X,Y); plt.draw(); # & predictor if 2D
        plt.pause(.01); # let OS draw the plot
        print("The loss for train with L1 Regularization is:", Jnll[-1])
        
    def trainL2(self, X, Y, alpha = 2, initStep=1.0, stopTol=1e-4, stopEpochs=5000, plot=None):
        """ Train the logistic regression using stochastic gradient descent  with L2 regularization"""
        M,N = X.shape;                     # initialize the model if necessary:
        self.classes = np.unique(Y);       # Y may have two classes, any values
        XX = np.hstack((np.ones((M,1)),X)) # XX is X, but with an extra column of ones
        YY = ml.toIndex(Y,self.classes);   # YY is Y, but with canonical values 0 or 1
        ...
        while not done:
            ...
            for i in np.random.permutation(M):
                ...
                #derive_nll = sigma_r - YY[i]
                derive_nll = -YY[i] * (1 - sigma_r) + (1 - YY[i]) * sigma_r
                regularization_vector = (alpha * 2)  * self.theta
                gradi = np.array([derive_nll, derive_nll * X[i,0], derive_nll * X[i,1]])
                gradi += regularization_vector
                self.theta -= stepsize * gradi;  # take a gradient step
            ...
            for i in range(X.shape[0]):
                ...
                if YY[i] > 0:
                    ...
                    Jsur += -np.log(1 - sigma_r)
            Jsur = Jsur / M
            Jsur += alpha * np.sum(np.square(self.theta))
            Jnll.append(Jsur) # TODO evaluate the current NLL loss
            #Moved the draw and plot to only have final result instead of every step
            #plt.figure(1); plt.plot(Jnll,'b-',J01,'r-'); plt.draw();    # plot losses
            #if N==2: plt.figure(2); self.plotBoundary(X,Y); plt.draw(); # & predictor if 2D
        ...
        if N==2: plt.figure(2); self.plotBoundary(X,Y); plt.draw(); # & predictor if 2D
        plt.pause(.01); # let OS draw the plot
        print("The loss for train with L2 Regularization is:", Jnll[-1])
