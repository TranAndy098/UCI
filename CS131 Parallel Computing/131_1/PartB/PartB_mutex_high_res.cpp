...
int main() {
    ...
    for (long unsigned int i = 0; i < thread_starttimes_hrc.size(); i++) {
        std::cout << "Thread with mutex took " << std::chrono::duration_cast<std::chrono::duration<double>>(thread_starttimes_hrc[i]-before_1_hrc).count() << " seconds to begin" << std::endl;
    }
